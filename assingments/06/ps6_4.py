...
def pick_best_word(hand, points_dict):
    ...
    for word in points_dict:
        if is_valid_word(word, hand, points_dict):
            results[word] = results.get(
                word, 0)+get_word_score(word, HAND_SIZE)
    high = 0
    bestword = ""
    r = ""
    for r in results:
...
def pick_best_word_faster(hand, rearrange_dict):
    ...
    """
    s = sort_word(
        hand)  # check to see if you can make a word with all letters in hand first
    if s in rearrange_dict:
        return rearrange_dict[s]
    d = {}
    bestword = "."
    bestscore = 0
    solve_subs(hand, d)
...
def sort_word(word):
    """return sting of word sorted from a-z
    """
    w = []
    i = 0
    for l in word:
    ...
    * If the user inputs anything else, ask them again.
    ...
    while True:
        if firstTime:
            firstTime = False
            cmd = raw_input(
                'Enter n to deal a hand and begin, or e to end game: ')
        else:
            cmd = raw_input(
