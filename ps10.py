...
class Hand(object):
	...
	def update(self, word):
		...
		for letter in self.handDict:
			_hand[letter] = self.handDict.get(letter, 0) - w.get(letter, 0)
		newhand = {}
		for letter in _hand:
	...
	def containsLetters(self, letters):
		...
		"""
		valid = True
		w = getFrequencyDict(letters)
		for letter in w:
			if self.handDict.get(letter, 0) < w.get(letter, 0): 
				valid = False
		return valid
	def isEmpty(self):
		"""
		...
		"""
		if not len(self.handDict):return True
		else:
			return False
	def __eq__(self, other):
		...
		for c in self.handDict:
			if c in other.handDict:
				if other.handDict[c] != self.handDict[c]:
					return False
		return True
	def __str__(self):
		"""
...
class Player(object):
	...
	def __cmp__(self, other):
		...
		they're equal.
		"""
		if self.points > other.getPoints():return 1
		if self.points < other.getPoints():return -1
		if self.points == other.getPoints():return 0
	...
	def pickBestWord(self, wordlist):
		...
		for word in wordlist.getList():
			if self.hand.containsLetters(word):
				results[word] = results.get(word,0)+getWordScore(word)
		high = 0
		bestword = ""
		r = ""
		for r in results:
